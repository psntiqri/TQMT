...
namespace Exilesoft.MyTime.Controllers
{
    public class AttendanceController : BaseController
    {
       ...
       //[DelphiAuthentication]
        public ActionResult Index(int? EmployeeId, string pickeddate)
        {
           ...
           EmployeeEnrollment loggedUser = db.EmployeeEnrollment.FirstOrDefault(a => a.EmployeeId == employeeId);
			...
			if (EmployeeId == null && ((User.IsInRole("Employee")) || (User.IsInRole("SystemAdmin"))))//loggedUser.Privillage <= 0)
        ...
        public JsonResult QuickFindLocation(int? employeeID, string employeeName, string selectedDate, string userType)
        {
            ...
            var d =Json(new { SearchResult = Repositories.AttendanceRepository.QuickFindLocation(employeeID, employeeName, selectedDate, userType, logedInUserId) });
       ...
       //[DelphiAuthentication]
        ...
        {
            ViewBag.EmployeeId = new SelectList(EmployeeRepository.GetAllEmployees(), "Id", "Name");
            return View();
        }
        ...
        public ActionResult Create(Attendance attendance)
        {
            ...
            ViewBag.EmployeeId = new SelectList(EmployeeRepository.GetAllEmployees(), "Id", "Name", attendance.EmployeeId);
            return View(attendance);
        }
                    ...
                    if (_employeeAttendanceList != null)
                    {
                        ...
                        if (_lastAttendance != null)
                        {
                            ...
                            if (_lastAttendance.InOutMode == "in")
                            {
                                ...
                                if (selectedDate.Value.Date == DateTime.Today.Date)
                                {
